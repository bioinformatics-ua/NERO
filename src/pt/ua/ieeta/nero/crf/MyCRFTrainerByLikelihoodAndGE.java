...
public class MyCRFTrainerByLikelihoodAndGE extends TransducerTrainer {

    private static Logger logger = LoggerFactory.getLogger(MyCRFTrainerByLikelihoodAndGE.class);
    ...
    public MyCRFTrainerByLikelihoodAndGE(CRF crf, ArrayList<GEConstraint> constraints, StateLabelMap map) {
        this.crf = crf;
    ...
    public boolean train(InstanceList trainingSet, InstanceList unlabeledSet, int numIterations) {
        ...
        if (initSupervised && supervisedCRF == null) {
            // train supervised
            if (numThreads == 1) {
            ...
            runEvaluators();
            supervisedCRF = new CRF(crf);
        }
        //crf = supervisedCRF;
        //logger.info("EQUALS: {}", crf.equals(supervisedCRF));
