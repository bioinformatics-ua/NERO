 ...
 *
 * Note that we aren't attending to the feature's value, and MALLET doesn't currently have any support at all for
...
public class InfoGain extends RankedFeatureVector {
    ...
    private static double[] calcInfoGains(InstanceList ilist) {
        ...
        for (int i = 0; i < ilist.size(); i++) {
            ...
            // Set labelling
            //LabelAlphabet ldict = (LabelAlphabet) inst.getTargetAlphabet();
            //inst.setTarget(ldict.lookupLabel(inst.getTarget()));
            
            Labeling labeling = inst.getLabeling();
            


            FeatureVectorSequence fsv = (FeatureVectorSequence) inst.getData();

            //FeatureVector fv = (FeatureVector) inst.getData ();

            for (int j = 0; j < fsv.size(); j++) {
                FeatureVector fv = fsv.get(j);
